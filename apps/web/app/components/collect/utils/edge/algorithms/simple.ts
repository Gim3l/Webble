...
export const getSimplePath = ({
  ...
  if (isDirectConnect) {
    ...
    if (isHorizontalLayout) {
      if (sourceOffset.x <= targetOffset.x) {
        ...
        return [
          { id: nanoid(), x: centerX, y: sourceOffset.y },
          { id: nanoid(), x: centerX, y: targetOffset.y },
        ];
      } else {
        const centerY = (sourceOffset.y + targetOffset.y) / 2;
        return [
          sourceOffset,
          { id: nanoid(), x: sourceOffset.x, y: centerY },
          { id: nanoid(), x: targetOffset.x, y: centerY },
          targetOffset,
        ];
      }
    } else {
  ...
  if (sourceDirection === targetDirection) {
    ...
    let point = { id: nanoid(), x: sourceOffset.x, y: targetOffset.y };
    const inStart = isInLine(point, source, sourceOffset);
    const inEnd = isInLine(point, target, targetOffset);
    if (inStart || inEnd) {
      point = { id: nanoid(), x: targetOffset.x, y: sourceOffset.y };
    } else {
